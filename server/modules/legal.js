...
function privacy() {
  ...
  return shell("/privacy", "Privacy policy",
    ...
    [
      ...
      ["Cookies and local storage", `<p>We use a small number of cookies and browser storage for: keeping you signed in, remembering your currency and recent searches, and counting visits anonymously.${process.env.GA4_ID || process.env.META_PIXEL_ID ? ` We also use ${[process.env.GA4_ID ? "Google Analytics" : "", process.env.META_PIXEL_ID ? "the Meta (Facebook/Instagram) Pixel" : ""].filter(Boolean).join(" and ")} to understand which pages and ads lead to bookings and to show our ads to people who visited our site. You can block these with your browser's privacy settings or an ad blocker, and manage ad preferences in your Google and Meta account settings.` : " We don't use advertising cookies."}</p>`],
